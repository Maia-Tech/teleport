...
- Regions must be provisioned with enough capacity to handle all traffic in
  the event of a complete region failure. This includes the ability to
  immediately absorb new traffic as nodes and users reconnect to the healthy regions.
...
<Admonition type="warning" title="Kubernetes CPU Limits">

We strongly discourage setting CPU limits on any Teleport component, as throttling can introduce unpredictable
latency, disrupt real-time authentication and networking operations, and trigger cascading failures across
otherwise healthy regions, potentially leading to a full cluster outage.

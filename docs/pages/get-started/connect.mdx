...
1. **Short-lived certificate issuance** - Teleport's Auth Service immediately issues you a short-lived SSH certificate (valid for 12 hours by default), signed by Teleport's internal Certificate Authority.
2. **The server trusts Teleport's CA** - When you enrolled the Ubuntu server, the Teleport Agent configured it to trust certificates signed by Teleport's CA instead of traditional SSH keys.
3. **Certificate-based authentication** - Your browser presents the certificate to the server, which validates it and grants access.

...
message ResourceConstraints {
  ...
  oneof details {
    AWSConsoleConstraints AWSConsole = 10 [(gogoproto.jsontag) = "aws_c"];
...
- **AWS Console ARN constraint**
  ```json
  {"cluster":"example.com","kind":"app","name":"aws-console","constraints":{"domain":1,"details":{"aws_c":{"arns":["arn:aws:iam::123456789012:role/Admin"]}}}}
  ```
- **SSH logins constraint**
  ```json
  {"cluster":"example.com","kind":"node","name":"bbbb56211-7b54-4f9e-bee9-b68ea156be5f","constraints":{"domain":3,"details":{"ssh":{"logins":["admin","web"]}}}}
  ```
...
- **Size limits**:  
  ...
  If a request exceeds these limits, it is rejected with a clear error message, and the user is advised to reduce the number of requested resources or split the request, similar to existing error behavior for long-term resource requests unable to be satisfied by a single Access List.
- **Safety**:  
...
#### Role Definition
Greedy 'deny' behavior is preserved, as `ResourceConstraints` are purely additive. If a role in a user's RoleSet has Deny rules blocking certain constraints (e.g., `role.spec.deny.logns`), those constraints are not presented as requestable, even if another role in the RoleSet would allow requesting or assuming them.
...
- Serialized `ResourceID`s with constraints will not be parsable by older versions. This is an intentional fail-closed design to avoid potential over-granting if constraints are ignored.
  - For now, to keep non-constraint-bearing `ResourceID`s compatible with older agents, we retain the existing slash-delimited string format for those, and only use the JSON format when constraints are present. The next major version could consider switching fully to JSON for all `ResourceID`s to simplify code paths.
